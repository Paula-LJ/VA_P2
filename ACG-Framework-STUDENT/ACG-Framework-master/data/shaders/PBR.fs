...
vec3 getPBR(PBRMat mat)
{
	...
	vec3 N = perturbNormal( normalize(v_normal), V, v_uv, texture2D(normalmap, texcoord).xyz);
	...
	if (u_has_pbr != 0){
		...
		vec3 G = min(1, (2*(NdotH)*(NdotV))/(VdotH), (2*(NdotH)*(NdotL))/(VdotH));
...
void main()
{
	...
	// 3. Shade (Direct + Indirect); 4. Apply Tonemapping; 
	vec4 color = getPBR(mat);

	//5. Any extra texture to apply after tonemapping; 
	color_textplus = vec4(color, 1.0); // FALTA AFEGIR UNBA TEXTURA EXTRA!!!

	//Last step: to gamma space
	color_gamma =  linear_to_gamma(color_textplus.xyz);
